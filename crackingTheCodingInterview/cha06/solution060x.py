"""
6.1
Add arithmetic operators (plus, minus, times, divide) to make the following expression true: 3 1 3 6 = 8. You can use any parentheses
you’d like.
...
Analyze:
Impossible, because the diagonally opposite corners are same color,
while each domino covers two different color, odd will not equal to
even.
...
Analyze:
* a lock is open except it is toggled odd times
* a lock will be toggled if its number can be divide by a number in
[1,100]
* usually, a number can be divided to two different factor, except the
square numbers, 1, 4, 9, ..., 100
* so if a lock is open only it is square number
so 10 lockers are open, and they are 1, 4, 9, ..., 100
"""
